	...
	*/
	public function handle_raidplan_delete($raidplan_id, &$raidplan_data)
	{
		...
		if (confirm_box(true))
		{
			...
			$meta_info = append_sid("{$phpbb_root_path}dkp.$phpEx", "mode=planner&amp;calM=".$date['month_no']."&amp;calY=".$date['year'].$etype_url_opts);
	...
	public function generate_calendar_smilies($mode)
	{
		...
		if ($mode == 'inline' && $display_link)
		{
			$template->assign_vars(array(
				...
				'U_MORE_SMILIES' 		=> append_sid("{$phpbb_root_path}dkp.$phpEx", 'mode=planneradd&amp;planmode=smilies'))
			);
		}
